...
Rref = diag(zCollector * zCollector') / learnLength;

args = zCollector;
targs = pCollector;
WoutAll = (inv(args * args' / learnLength + ...
    TychWouts(nNet) * eye(M)) * args * targs' / learnLength)' ;
Wout = WoutAll(end,:);
...
for nNet = 1:NNets
    ...
    MismatchRatiosTest{nNet} = zeros(M);
end
...
for n = 1:washoutLength
    rs{1} = tanh(G * zs{1} + Win * testPatt(1,n)...
        + Wfb * yAll{1} + bias);
    zs{1} = C{1} .* (F * rs{1});
    yAll{1} = WoutAll * zs{1};
    for nNet = 2:NNets
